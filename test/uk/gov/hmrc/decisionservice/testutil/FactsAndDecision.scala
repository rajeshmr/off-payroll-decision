...
case class FactsAndDecision(request:Facts, expectedDecision:CarryOver)

object FactsAndDecision {
  private val NUM_OF_CLUSTER_RESULT_COLUMNS = 3
  def read(path:String):Try[List[FactsAndDecision]] = {
    tokenize(path).map { tokens =>
      ...
      scenarios.map { scenario =>
        ...
        FactsAndDecision(Facts(tagNames.zip(answers.map(>>>(_))).toMap), >>>(rest))
      }
    }
  }
}
